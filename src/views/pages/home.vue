<template>
  <div>
    ...
    <v-slide-group v-model="model" class="pa-4" active-class="success" show-arrows>
      <v-slide-item v-for="post in posts" :key="post.id" v-slot:default="{ active, toggle }">
        <v-card
          :color="active ? undefined : 'white lighten-1'"
          class="ma-4"
          width="300"
          @click="toShowOne(post.id)"
        >
          <img
    ...
    <v-container>
      <v-row>
        <v-col v-for="post in posts" :key="post.id" cols="12" sm="3">
          <v-card class="mx-auto" max-width="300">
            ...
            <v-card-actions>
              <v-btn block color="blue" text @click="toShowOne(post.id)">معاينة الخبر</v-btn>
            </v-card-actions>
          </v-card>
        </v-col>
      </v-row>
    </v-container>
    <!-- الاخبار المقطع الثاني -->
    <div class="divider">
      <span></span>
      <span>الاقتصاد</span>
      <span></span>
    ...
    <v-container>
      <v-row>
        <v-col v-for="post in posts3" :key="post.id" cols="12" sm="3">
          <v-card class="mx-auto" max-width="300">
            <v-img
              class="white--text align-end mb-3"
              height="200px"
    ...
    <v-container>
      <v-row>
        <v-col v-for="post in posts4" :key="post.id" cols="12" sm="3">
          <v-card class="mx-auto" max-width="300">
            <v-img
              class="white--text align-end mb-3"
              height="200px"
    ...
    <v-container>
      <v-row>
        <v-col v-for="post in posts5" :key="post.id" cols="12" sm="3">
          <v-card class="mx-auto" max-width="300">
            <v-img
              class="white--text align-end mb-3"
              height="200px"
...
export default {
  ...
  methods: {
    ...
    showPosts2() {
      // this.loading = true;
      Axios.get("post?SectionId=fe22da2e-9b47-44e4-911c-3e225ac69e76&Take=8&Skip=0", {
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
          Authorization: "Bearer " + this.$store.state.idToken
    ...
    showPosts3() {
      // this.loading = true;
      Axios.get("post?SectionId=680dbcab-7a33-47ca-9fea-0b2f19ae11f6&Take=8&Skip=0", {
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
          Authorization: "Bearer " + this.$store.state.idToken
    ...
    showPosts4() {
      // this.loading = true;
      Axios.get("post?SectionId=2f6d3098-ede5-4125-b8e5-6131327b876d&Take=8&Skip=0", {
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
          Authorization: "Bearer " + this.$store.state.idToken
    ...
    showPosts5() {
      // this.loading = true;
      Axios.get("post?SectionId=24c15152-ec93-4fb7-b94b-ffab71b8f64f&Take=8&Skip=0", {
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
          Authorization: "Bearer " + this.$store.state.idToken
